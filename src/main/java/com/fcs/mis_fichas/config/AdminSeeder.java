 ...
 * Se ejecuta al arrancar la aplicación y, solo si no existe un ADMIN activo:
 * - Crea el usuario ADMIN con las credenciales del .env
 * - Carga las categorías y subcategorías de sistema
 * - En perfil developer/dev, ejecuta data-fake.sql para poblar usuarios y transacciones de prueba
 */
@Configuration
...
public class AdminSeeder {
     ...
     * Si no existe un usuario ADMIN con status ACTIVE, crea el admin, carga las
     * categorias/subcategorias de sistema y, solo en perfil DEV, ejecuta el SQL
     * de data-fake.sql para poblar usuarios y transacciones de prueba.
     *
    ...
    CommandLineRunner seedAdmin() {
        return args -> {
            if (activeAdminExists()) {
                log.info("Ya existe un ADMIN activo. Se omite la carga inicial de datos.");
                return;
            }
    ...
    /**
     * Verifica si ya existe un usuario ADMIN con status ACTIVE y no eliminado.
     *
     * @return true si existe un admin activo con las credenciales configuradas
     */
    private boolean activeAdminExists() {
        return userRepository.findByEmailAndDeletedAtIsNull(adminEmail)
                .map(user -> user.getRole() == Role.ADMIN && user.getStatus() == Status.ACTIVE)
                .orElse(false);
    }

    /**
     * Indica si la aplicación se está ejecutando con el perfil developer/dev.
     *
     * @return true si el perfil activo es developer o dev
     */
    private boolean isDeveloperProfile() {
        String[] activeProfiles = environment.getActiveProfiles();
        return activeProfiles != null && (
                Arrays.asList(activeProfiles).contains("developer") ||
                Arrays.asList(activeProfiles).contains("dev")
        );
    }

    /**
     * Crea el usuario ADMIN por defecto con las credenciales del .env.
    ...
    // ========================================================================
    // FAKE DATA — Solo se ejecuta en perfil DEVELOPER/DEV
    // ========================================================================

    /**
     * Ejecuta el script data-fake.sql línea por línea para poblar usuarios y
     * transacciones de prueba. Solo debe invocarse cuando la app arranca con el
     * perfil developer o dev.
     */
    private void executeFakeDataSql() {
        log.info("Perfil DEV detectado — cargando fake data desde data-fake.sql...");
        Resource resource = new ClassPathResource("data-fake.sql");
        if (!resource.exists()) {
            log.warn("No se encontró data-fake.sql en el classpath. Se omite la carga de datos fake.");
            return;
        }
        ...
        try (InputStream is = resource.getInputStream();
            ...
            log.info("Fake data cargada: {} sentencias SQL ejecutadas desde data-fake.sql.", executed);
        } catch (IOException e) {
            throw new RuntimeException("Error al leer o ejecutar data-fake.sql", e);
        }
    }
}
